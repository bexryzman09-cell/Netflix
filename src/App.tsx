...
const GENRES = [
  ...
  'Романтика', 'Фантастика', 'Приключения'
]
...
function App() {
  ...
  const watchedCount = MOVIES.filter(
    m => localStorage.getItem(`watched-${m.title}-${m.year}`) === 'true'
  ).length
  ...
  const filteredMovies = randomMovies
    .filter(movie => {
      ...
      const matchesFilter =
        contentFilter === 'all'
        ||
        (contentFilter === 'movie' && movie.type === 'movie')
        ||
        (contentFilter === 'series' && movie.type === 'series')
        ||
        (contentFilter === 'anime' && movie.type === 'anime')
        ||
        (
          contentFilter === 'watched'
          &&
          localStorage.getItem(`watched-${movie.title}-${movie.year}`) === 'true'
        )
      ...
      return matchesSearch && matchesFilter && matchesGenre && matchesYear && matchesRating
    })
    .sort((a, b) => {
  ...
  return (
    ...
    <div className="bg-black text-white movie-page">
      ...
      {heroMovie && (
        <HeroBanner
          movie={heroMovie}
          onWatch={() => handleSelectMovie(heroMovie, "movie")}
          onFavorite={() => toggleFavorite(heroMovie)}
      ...
      <header className="movie-header">
        ...
        <span className="info-icon dark:text-white text-black">ⓘ</span>

      </header>

      <PopularRow movies={popularMovies} onSelect={handleSelectMovie} />
      ...
      <div className="movie-filters">
        ...
        <div className="filter-tabs">
          ...
          <button
            className={contentFilter === 'all' ? 'filter-tab active' : 'filter-tab'}
          ...
          <button
            className={contentFilter === 'movie' ? 'filter-tab active' : 'filter-tab'}
          ...
          <button
            className={contentFilter === 'series' ? 'filter-tab active' : 'filter-tab'}
          ...
          <button
            className={contentFilter === 'anime' ? 'filter-tab active' : 'filter-tab'}
          ...
          <button
            className={contentFilter === 'watched' ? 'filter-tab active' : 'filter-tab'}
      ...
      <div className="advanced-filters">
        ...
        >
          ...
          <option value="1990s">1990-е</option>
          <option value="before1990">До 1990</option>
        </select>
        ...
        <div className="genre-pills-row">
          ...
          <button
            className={genreFilter === 'all' ? 'genre-filter-btn active' : 'genre-filter-btn'}
            onClick={() => { setGenreFilter('all'); setVisibleMovies(8) }}
          >
            Все жанры
          </button>
          ...
          {GENRES.map((genre) => (
            <button
              key={genre}
              className={genreFilter === genre ? 'genre-filter-btn active' : 'genre-filter-btn'}
              onClick={() => { setGenreFilter(genre); setVisibleMovies(8) }}
            >
              {genre}
            </button>
          ))}
